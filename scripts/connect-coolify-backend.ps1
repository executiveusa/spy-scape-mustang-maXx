...
if ($UpdateEnv) {
  ...
  $envPayload = @{
    data = @(
      ...
      @{ key = 'MAXX_HERMES_HOME'; value = '/runtime/hermes'; is_literal = $true; is_multiline = $false; is_preview = $false },
      @{ key = 'MAXX_HERMES_VENDOR_PATH'; value = '/opt/hermes-agent'; is_literal = $true; is_multiline = $false; is_preview = $false },
